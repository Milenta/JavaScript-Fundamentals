...
function calcAge1(birthYear) {
  // create a function to calculate your age
...
const calcAge2 = function (birthYear) {
  // this is called anonimus function. it is called and stored in value
  return 2024 - birthYear;
};
...
const yearsUntilRetirenment = (birthYear, retirenmentAge) =>
  birthYear + retirenmentAge - 2024;
...
function cutFruitPieces(fruit) {
  // funtion to cut fruit in 4 pieces
  return fruit * 4;
}

function fruitProcessor2(apples, oranges) {
  console.log(apples, oranges);
  const juice = `Juice is made with ${apples} apples - ${cutFruitPieces(
    apples
  )} pieces, and ${oranges} oranges - ${cutFruitPieces(oranges)} pieces.`;
  return juice;
}
...
const agesAray = [
  calcAgeForArray(yearsArray[0]),
  calcAgeForArray(yearsArray[1]),
  calcAgeForArray(yearsArray[calcAgeForArray.length - 1]),
]; // koristimo funkcije da dobijemo varijable
...
const bob = {
  ...
  calcAgeObject: function (birthYear) {
    return 2024 - birthYear;
  }, // objekat moze da sadrzi function expression kao vrednost - onda mu se pristupa sa bob.calcAgeObject(1989)
  calcAgeObject2: function () {
    return 2024 - this.birthYear;
  }, // objekat moze da sadrzi function expression kao vrednost - onda mu se pristupa sa bob.calcAgeObject()
  ...
  getSummery: function () {
    return `${this.firstName} is a ${this.calcAgeObject3()} year old ${
      this.job
...
const interestedIn = prompt(
  "What do you want to know about Bob? firstName, lastName, age, job or friends?"
);
...
if (bob[interestedIn]) {
  ...
  console.log(
    `${interestedIn} is wrong request. Try with firstName, lastName, age, job or friends`
  );
}
...
console.log(
  `${bob.firstName} has ${bob.friends.length} friends. And his best friend is ${bob.friends[0]}`
);
...
for (let i = 0; i < forLoopArray.length; i++) {
  ...
  console.log(
    "for loop create new array " + forLoopArray[i],
    typeof forLoopArray[i]
  );
}
...
for (let exercise = 0; exercise < exerciseArray.length; exercise++) {
  ...
  for (let i = 1; i < 11; i++) {
    // loop 10 timmes for each element of array
...
let rep = 1;
while (rep <= 5) {
  // while condition is true loop is running
